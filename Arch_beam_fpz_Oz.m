function [F3,F3_ori,X,Y,CM_ori,NI_ori,TT_ori] = Arch_beam_fpz_Oz(Nz,Iz,M1,M2,Cz,msh_path)
%====================================================================================================================
% This function is used to get the SGP F3 based on the SGP parameters.
% [input]:
...
%       CM_ori: circumference
%       NI_ori: distance from nasion to inion
%       TT_ori: distance from left tragus to right tragus
%
...
plot3(EdgePts(:,1),EdgePts(:,2),EdgePts(:,3),'.','color','b','markersize',10)
EdgePts = data_smooth(Nz,Iz,M1,M2,EdgePts,1000);
plot3(EdgePts(:,1),EdgePts(:,2),EdgePts(:,3),'.','color','r','markersize',10)
TT_ori = cumsum(sqrt(sum(diff(EdgePts*(Vmax+Vmin)/100-Vmin).^2,2)));
TT_ori = TT_ori(end);

[X,Y] = Beam_F3(TT_ori,NI_ori,CM_ori);
...
F3_ori = Ypoint;
Ypoint = 100*(Ypoint + Vmin)/(Vmax+Vmin);
